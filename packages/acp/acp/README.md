...
| `session/new` | Creates a fresh agent with an absolute primary `cwd`; empty `additionalDirectories` and `mcpServers` are accepted, non-empty values reject. Advertises the configured provider's model catalog as a stable `configOptions` `model` selector (and, for Buzz-compatible clients, an extra `models` state block); a client may switch models per session via `session/set_config_option`. |
| `session/set_config_option` | Updates the session's live model when `configId` is `model` and `value` is a model id; the next model step routes through the new selection without restarting the agent. Other config ids are acknowledged without effect. Returns the refreshed `configOptions`. |
