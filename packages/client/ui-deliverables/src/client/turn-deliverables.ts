 ...
 * @param downloadFile - The chat view's file downloader.
 * @param label - Localizes the accessible download-label for a resolved path.
...
export function producedFileMentions(
  ...
  return {
    resolve(value) {
      ...
      return { open: () => { downloadFile(path) }, label: label(path), title: path }
    },
  }
}
